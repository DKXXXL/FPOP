...
Closing Fact natcst_inj:
  forall n m,
    ...
    n = m 
    by {intros n m h; inversion h;  eauto}.


Overridable pins {exp} 
Field is_natcst_do : option self__LangCP.exp -> (nat -> option self__LangCP.exp) -> option self__LangCP.exp
:= fun e f =>
    match e with 
    | Some (self__LangCP.__internal_natcst n) => f n 
    | _ => None
    end.
...
Overridable pins {exp is_natcst_do}  
Field 
is_natcst_do_axiom2 :
  forall e f v,
...
destruct e; try (cbn in *; inversion h; fail); cbn in *; simpl in h. 
eexists; eauto. 
Qed. FEnd is_natcst_do_axiom2. 

Inherit Until Field is_strcst_do.

Override pins {exp} 
Field is_strcst_do : option self__LangCP.exp -> (string -> option self__LangCP.exp) -> option self__LangCP.exp
:= fun e f =>
    match e with 
    | Some (self__LangCP.__internal_strcst n) => (f n) 
    | _ => None 
    end.

Inherit Until Field is_strcst_do_axiom2.

Override pins {exp is_strcst_do}  
Field
is_strcst_do_axiom2 :
  forall e f v,
  self__LangCP.is_strcst_do e f = Some v ->
  exists s, e = Some (self__LangCP.strcst s).
FProof.
intros e f v h; destruct e as [e |]; cbn in *; try discriminate. 
destruct e; try (cbn in *; inversion h; fail); cbn in *; simpl in h. 
eexists; eauto. 
...
Inherit Until Field exp_eval_cases_handler.
...
Field natcst : 
  forall n, self__exp_eval_cases_handler.motive (self__LangCP.natcst n) :=
  fun n env => Some (self__LangCP.natcst n).


(* We have a horrible add hahaha *)
Field add : 
  forall e1 (rec1 : self__exp_eval_cases_handler.motive e1) e2 (rec2 : self__exp_eval_cases_handler.motive e2), self__exp_eval_cases_handler.motive (self__LangCP.add e1 e2) :=
  fun e1 rec1 e2 rec2 env =>
  self__LangCP.is_natcst_do 
      (rec1 env) 
      (fun n1 => 
      self__LangCP.is_natcst_do 
          (rec2 env) 
          (fun n2 => Some (self__LangCP.natcst (n1 + n2)))).
...
FInductive literals : Set :=
| natlit : nat -> literals
| otherv : literals.

Closing Fact natlit_inj:
  forall n m,
    self__AI.natlit n = self__AI.natlit m ->
    n = m 
    by {intros n m h; inversion h;  eauto}.

Closing Fact literals_eq_dec :
  forall (x y : self__AI.literals), {x = y} + {x <> y}
  by plain transparent {
    intros x y;
     repeat (decide equality)
  }.

Override Field absExp : Type := self__AI.literals.

Override pins {absExp literals}
Field AEjoin : self__AI.absExp -> self__AI.absExp -> self__AI.absExp :=
  fun e1 e2 => 
    match e1, e2 with 
    | self__AI.__internal_natlit a, self__AI.__internal_natlit b => if eq_nat_dec a b then self__AI.natlit a else self__AI.otherv
    | _, _ => self__AI.otherv
    end.
...
Override pins {absExp AEle literals}
Field 
...
intros; destruct_ALL; (try (repeat eexists; eauto; fail)).
Qed. FEnd le_trans.


Override pins {absExp AEle AEjoin literals}
Field join_le_left :
  forall x y, self__AbsExpData.le x (self__AbsExpData.join x y).
FProof.
unfold self__AbsExpData.le.  unfold self__AI.AEle.  

unfold self__AbsExpData.join. unfold self__AI.AEjoin.  intros.
destruct x; destruct y; (try eexists; eauto).

destruct (eq_nat_dec n n0); subst. 
left; eexists; eauto. right ;eauto. 
...
unfold self__AI.AEle. intros x y h1 h2.
destruct h1; destruct h2; destruct_ALL; subst; eauto.
...
intros. destruct l; destruct H; destruct_ALL; subst; eauto;  eauto; cbn in *. 
...
Override pins {absExp}
Field RExp : self__LangCP.exp -> self__AI.absExp -> Prop
  := fun e ae =>
    (forall n, e = self__LangCP.natcst n -> ae = self__AI.natlit n \/ ae = self__AI.otherv).
...
Field motive : self__LangCP.exp -> Type := 
  fun e => 
...
Field var : 
  forall (i : ident), self__exp_analyze_handler.motive (self__LangCP.var i). 
FProof.
unfold self__exp_analyze_handler.motive.
intros.
destruct (self__AI.alookup amem i) eqn:h1. exists t.
frec_eval self__LangCP.exp_eval. unfold self__LangCP.exp_eval_cases_handler.var. intros. unfold self__AI.RMemory in *.
forwards*: H; destruct_ALL. rewrite h1 in H1. inversion H1; subst; eauto.
exists self__AI.otherv.
frec_eval self__LangCP.exp_eval. unfold self__LangCP.exp_eval_cases_handler.var. intros.  unfold self__AI.RMemory in *.
...
Overridable pins {absExp RExp}
Field boolcst :
  forall (b : bool), self__exp_analyze_handler.motive (self__LangCP.boolcst b). 
FProof.
intros x mem. exists self__AI.otherv. unfold self__AI.RMemory. frec_eval self__LangCP.exp_eval. unfold self__LangCP.exp_eval_cases_handler.boolcst. unfold self__AI.RExp.  intros. inversion H0; subst; eauto. 
Qed. FEnd boolcst.
...
Overridable pins {absExp RExp}
Field strcst :
  forall n : string, self__exp_analyze_handler.motive (self__LangCP.strcst n).
FProof. 
intros n mem; intros.
exists self__AI.otherv. unfold self__AI.RMemory. frec_eval self__LangCP.exp_eval. unfold self__LangCP.exp_eval_cases_handler.strcst. unfold self__AI.RExp.  intros. inversion H0; subst; eauto.
Qed. FEnd strcst.

Overridable pins {absExp RExp}
Field concat :
forall (e1 : self__LangCP.exp) (rec1 : self__exp_analyze_handler.motive e1) 
       (e2 : self__LangCP.exp) (rec2 : self__exp_analyze_handler.motive e2), self__exp_analyze_handler.motive (self__LangCP.concat e1 e2).
FProof. 
intros. intros mem; intros.
exists self__AI.otherv. unfold self__AI.RMemory. frec_eval self__LangCP.exp_eval. unfold self__LangCP.exp_eval_cases_handler.strcst. unfold self__AI.RExp. intros; subst; eauto.
Qed. FEnd concat. 

Overridable pins {absExp RExp}
Field natcst :
  forall n : nat, self__exp_analyze_handler.motive (self__LangCP.natcst n).
FProof. intros n mem.
exists (self__AI.natlit n).  unfold self__AI.RMemory. frec_eval self__LangCP.exp_eval. unfold self__LangCP.exp_eval_cases_handler.natcst. unfold self__AI.RExp. intros; subst; eauto. inversion H0; eauto. forwards*: self__LangCP.natcst_inj. 
Qed. FEnd natcst.

Overridable pins {absExp RExp literals}
Field add :
forall e1 (rec1 : self__exp_analyze_handler.motive e1) e2 (rec2 : self__exp_analyze_handler.motive e2), self__exp_analyze_handler.motive (self__LangCP.add e1 e2).
FProof. unfold self__exp_analyze_handler.motive.
...
intros. 
frec_eval self__LangCP.exp_eval. unfold self__LangCP.exp_eval_cases_handler.add in *.
...
self__LangCP.exp -> self__AI.absMemory ->  self__AI.absExp :=
...
Definition analyze_end (fuel : nat) (c : LangCP.stmt) (i : ident) : option LangCP.AI.absExp :=
  let initialst := LangCP.AI.injSt (c, []) (LangCP.AI.ABSMemory.M.empty _) in 
  let result := LangCP.AI.analyze fuel initialst in
 
  match LangCP.AI.ABSState.M.find (LangCP.nop, []) result with 
  | None => None 
  | Some mem => LangCP.AI.ABSMemory.M.find i mem 
  end.
...
Definition testcase1 := fun (_ : unit) => 
    (analyze_end 5 
      (assign 1 (natcst 1)) 
      1).

Definition testcase2 := fun (_ : unit) => 
    (analyze_end 5 
      (seq (assign 1 (natcst 1))
           (assign 1 (add (var 1) (natcst 2)))
          ) 
      1).
